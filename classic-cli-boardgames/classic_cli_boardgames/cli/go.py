"""Go CLI loop and input handling."""
...
from classic_cli_boardgames.cli.help_text import GO_HELP
from classic_cli_boardgames.core.interfaces import AIConfig
from classic_cli_boardgames.games.go import Color, GoEngine, parse_move_input

_ENGINE = GoEngine()
_AI = GoAI(_ENGINE)


def launch_go_game() -> bool:
    """Run a Go game loop. Returns True if the program should exit."""
    difficulty, command = prompt_difficulty("Go")
    ...
    player_mode, command = prompt_player_mode("Go")
    if command is NavCommand.BACK:
        print("Returning to the main menu.")
        print()
        return False
    ...
    if ai_vs_ai:
        print("Starting Go. Computer vs Computer.")
        ...
        print(f"Starting Go. You play as {human_color.label}.")
        print(f"Computer difficulty: {difficulty.name}. Type 'help' for commands.")
    while True:
        print()
        ...
        raw = input(f"Go ({state.active_color.label})> ")
        command = parse_navigation_command(raw)
        if command is not None:
            if command is NavCommand.HELP:
                print(GO_HELP)
